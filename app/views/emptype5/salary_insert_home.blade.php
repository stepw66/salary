...
<div class="">
  <div class="medium-8 columns">
    <h3>ลูกจ้างรายวัน : บันทึกเงินเดือน</h3>
  </div>
  <div class="medium-4 columns">
     {{ Form::open( array( 'url' => 'emptype5/salary_insert-search' ) ) }}
      <div class="row collapse">   
          <div class="small-8 medium-8 columns">
            <input type="search" class="search" id="search_salary_insert5" name="search_salary_insert5" placeholder="ค้นหา CID, ชื่อ, นามสกุล"/>
          </div>
   ...
   <div class="large-12 columns">
      ...
      <?php $i=0; ?>
        ...
        <tr> 
          ...
          <?php if( $a->salary != '' ) { ?>
          <td><a  title="แก้ไขข้อมูล" data-reveal-id="Modalsalary_insert5" onclick="Modalsalary_insert5( {{ $a->cid }}, '{{ $a->pname }}', '{{ $a->fname }}', '{{ $a->lname }}', '{{ $a->bank }}', '{{ $a->bank_acc_id }}', '{{ $a->bank_acc }}', {{ $a->salary }}, '{{ $a->salary_other }}', '{{ $a->salary_sso }}', '{{ $a->salary_cpk }}', '{{ $a->tax_id }}' )" href=""><i class="fi-page-edit small"></i></a></td>                        
          <?php } ?>
          <?php if( $a->salary == '') { ?>
          <td>-</td>
          <?php } ?> 
...
<div id="Modalsalary_insert5" class="reveal-modal" data-reveal>
  <h3>บันทึกเงินเดือน : <span id="Modalsalary_insertTitle5"></span></h3>

  <div id="fromsalary_insert5"></div>

...
<ul class="breadcrumbs">
  ...
  <li class="unavailable"><a href="#">ข้าราชการ</a></li>
  <li class="current"><a href="#">บันทึกเงินเดือน</a></li>
</ul>

<div class="">
  <div class="medium-8 columns">
    <h3>ข้าราชการ : บันทึกเงินเดือน</h3>
  </div>
  <div class="medium-4 columns">
     {{ Form::open( array( 'url' => 'emptype3/salary_insert-search' ) ) }}
      <div class="row collapse">   
          <div class="large-8 small-9 columns">
            <input type="search" class="search" id="search_salary_insert3" name="search_salary_insert3" placeholder="ค้นหา CID, ชื่อ, นามสกุล"/>
          </div>
   ...
   <div class="large-12 columns">
      ...
      <?php $i=0; ?>
        ...
        <tr> 
          ...
          <?php if( $a->salary != '' ) { ?>
          <td><a  title="แก้ไขข้อมูล" data-reveal-id="Modalsalary_insert3" onclick="Modalsalary_insert3( {{ $a->cid }}, '{{ $a->pname }}', '{{ $a->fname }}', '{{ $a->lname }}', '{{ $a->bank }}', '{{ $a->bank_acc_id }}', '{{ $a->bank_acc }}', '{{ $a->salary }}', '{{ $a->r_c }}', '{{ $a->special }}', '{{ $a->son }}', '{{ $a->kbk }}', '{{ $a->tax }}', '{{ $a->tax_id }}' )" href=""><i class="fi-page-edit small"></i></a></td>                        
          <?php } ?>
          <?php if( $a->salary == '') { ?>
          <td>-</td>
          <?php } ?>    
...
<div id="Modalsalary_insert3" class="reveal-modal" data-reveal>
  <h3>บันทึกเงินเดือน : <span id="Modalsalary_insertTitle3"></span></h3>

  <div id="fromsalary_insert3"></div>
